  ...
  function svgPlaceholder(item) {
    const xmlEscape = (v) =>
      String(v || "")
        ...
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;");
    const category = xmlEscape(String(item?.category || "ACCESSORY").toUpperCase());
    const sku = xmlEscape(String(item?.id || "0000"));
    const nameRaw = String(item?.name || "Premium Mobile Product").trim().replace(/\s+/g, " ");
...
<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1200 1200'>
  ...
  function safeProductImage(item) {
    const src = String(item?.image || "");
    return hasSuspiciousImagePath(src) ? svgPlaceholder(item) : src;
  }
        ...
        .map(
          (item) => `
            <article class="twm-cart-item">
              <img src="${esc(safeProductImage(item))}" alt="${esc(item.name || "Product")}" />
    ...
    function addToCart(item, qty = 1) {
      ...
      if (existing) {
        ...
        cart.push({
          ...
          image: String(safeProductImage(item)),
