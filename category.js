(() => {
  ...
  function pageTitle(filterKey, subKey) {
    return subcatLabel(filterKey, subKey) || titleByKey(filterKey);
  }
  ...
  function breadcrumbFor(filterKey, subKey) {
    ...
    if (root === "FUNDAS") {
      if (sub === "IPHONE") {
        return "Start / Brand / Apple / iPhone";
      }
      if (/^IPHONE_/.test(sub)) {
        return `Start / Brand / Apple / iPhone / ${label}`;
      }
    ...
    if (root === "SIM") {
      return `Start / SIM / ${label}`;
    }

    if (root === "OFERTA" || root === "OFFERS") {
      return `Start / Offers`;
    }
